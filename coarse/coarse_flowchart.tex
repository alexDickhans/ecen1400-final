...
\node [process, below=of has_cmd, yshift=-0.5cm] (check_btn) {Process Buttons};
\node [decision, below=of check_btn] (is_pressed) {Pressed?};
\node [process, right=of is_pressed, xshift=0.5cm] (send) {Send Command};

\node [process, below=of is_pressed, yshift=-0.5cm] (haptic) {Handle Haptic};
...
\draw [arrow] (play) |- (check_btn);
\draw [arrow] (has_cmd) -- node[anchor=east] {No} (check_btn);

% Button branch
\draw [arrow] (check_btn) -- (is_pressed);
\draw [arrow] (is_pressed) -- node[anchor=south] {Yes} (send);
\draw [arrow] (send) |- (haptic);
\draw [arrow] (is_pressed) -- node[anchor=east] {No} (haptic);
